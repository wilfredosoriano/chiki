/**
 * Add Loan modal — record a new loan with amount, payment schedule, and due date.
 * Basic fields are always visible; optional details are tucked behind "More details".
 */
...
import { insertLoan, getAllLoans } from '@/db/loanQueries';
...
function calcMonthlyPayment(
  ...
  if (addOn) {
    return (principal + principal * r * termMonths) / termMonths;
  }
...
export default function AddLoanScreen() {
  ...
  async function handleSave() {
    ...
    const existingLoans = await getAllLoans(db);
    const activeLoans = existingLoans.filter((l) => l.isActive);
    if (!isPremium && activeLoans.length >= FREE_TIER_LIMITS.MAX_LOANS) {
      Alert.alert(
        'Loan limit reached',
        `Free plan includes up to ${FREE_TIER_LIMITS.MAX_LOANS} active loans. Upgrade to Premium for unlimited loans.`,
        [
    ...
    if (isNaN(parsedRemaining) || parsedRemaining < 0) {
      Alert.alert('Invalid balance', 'Remaining balance cannot be negative.');
      return;
    }
    ...
    try {
      ...
      Alert.alert('Error saving loan', msg);
    } finally {
      setSaving(false);
    }
  }
      ...
      >
        ...
        <View style={{ gap: spacing.xs }}>
          ...
          <TextInput
            ...
            placeholder="e.g. SSS Loan, Car Loan, Paluwagan"
        ...
        <View style={{ gap: spacing.xs }}>
          ...
          <TextInput
            ...
            placeholder="e.g. BDO, Metrobank, SSS, Nanay"
            placeholderTextColor={colors.textTertiary}
            style={[styles.input, {
              backgroundColor: colors.surface, borderColor: colors.border,
              color: colors.textPrimary, borderRadius: radius.md, fontSize: typography.sizes.base,
        ...
        <View style={{ gap: spacing.xs }}>
          ...
          <Text style={{ color: colors.textTertiary, fontSize: typography.sizes.xs }}>
            Not sure? Fill in interest rate + term in "More details" to calculate it.
          </Text>
        </View>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ flexGrow: 0 }}>
            <View style={{ flexDirection: 'row', gap: spacing.sm, paddingVertical: 4 }}>
                ...
                return (
                  ...
                  >
                    <Text style={{
                      ...
                      fontWeight: '700', fontSize: typography.sizes.sm,
                    }}>
              ...
              <View style={[styles.dayChip, {
                ...
                borderRadius: radius.full, minWidth: 72,
              }]}>
                <TextInput
                  value={dueDayInput}
        ...
        <TouchableOpacity
          ...
          style={[styles.detailsToggle, {
            ...
            borderColor: showDetails ? colors.accent + '60' : colors.border,
        ...
        >
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, flex: 1, marginRight: 8 }}>
            <Ionicons
              ...
              size={16}
              color={showDetails ? colors.accent : colors.textSecondary}
            />
            <Text style={{ color: showDetails ? colors.accent : colors.textSecondary, fontWeight: '600', fontSize: 14 }}>
              {showDetails ? 'Less details' : 'More details'}
            </Text>
            <Text style={{ color: colors.textTertiary, fontSize: 12, flex: 1 }} numberOfLines={1}>
              — interest rate, remaining balance, end date…
            </Text>
          </View>
          <Ionicons
          ...
          <View style={{ gap: spacing.lg }}>
            ...
            <View style={{ gap: spacing.xs }}>
              ...
              <View style={[styles.toggleRow, {
                backgroundColor: colors.surfaceElevated,
                borderColor: colors.border, borderRadius: radius.md, marginTop: 2,
              }]}>
                {([
              ...
              <Text style={{ color: colors.textTertiary, fontSize: typography.sizes.xs }}>
                {addOn
                  ? 'Interest charged on the full loan amount every month — your monthly payment stays the same from start to finish.'
                  : 'Interest charged only on what you still owe — so you pay less interest as you pay it down.'}
              </Text>
            </View>
            ...
            <View style={{ gap: spacing.xs }}>
              ...
              <Text style={{ color: colors.textTertiary, fontSize: typography.sizes.xs, lineHeight: 16 }}>
                Fill in Principal + Interest Rate + Term → tap the button to auto-fill the correct monthly payment.
              </Text>
            </View>
            ...
            <View style={{ gap: spacing.xs }}>
              <View style={styles.labelRow}>
                ...
                <View style={[styles.optionalBadge, { backgroundColor: colors.surfaceSecondary }]}>
                  <Text style={{ color: colors.textTertiary, fontSize: 10, fontWeight: '600' }}>LEAVE BLANK = SAME AS ABOVE</Text>
                </View>
              </View>
              <View style={[styles.amountRow, {
                backgroundColor: colors.surface, borderColor: colors.border, borderRadius: radius.md,
              ...
              <View style={[styles.toggleRow, {
                ...
                {([
                  ...
                  <TouchableOpacity
                    ...
                    style={[styles.toggleBtn, {
                      backgroundColor: isRecurring === opt.value ? selectedColor : 'transparent',
                      borderRadius: radius.sm,
                    }]}
                  >
                    <Text style={{
                      color: isRecurring === opt.value ? '#fff' : colors.textSecondary,
                      fontWeight: '700', fontSize: typography.sizes.sm,
                    }}>
                      {opt.label}
                    </Text>
        ...
        {/* ── Amortization Summary Card — shows outside collapsible so it's always visible once filled ── */}
              ...
              {amortSummary.totalInterest != null && amortSummary.totalInterest > 0 && (
                <>
                  ...
                  <View style={styles.summaryRow}>
                    ...
                    <Text style={{ color: colors.textPrimary, fontSize: 13, fontWeight: '600' }}>
                      ₱{fmt(parseFloat(principal.replace(/,/g, '')))}
                    </Text>
                  </View>
                  <View style={styles.summaryRow}>
                    <Text style={{ color: colors.expense, fontSize: 13 }}>Total interest</Text>
                    <Text style={{ color: colors.expense, fontSize: 13, fontWeight: '700' }}>
                      +₱{fmt(amortSummary.totalInterest)}
                    </Text>
                  </View>
                </>
              )}
...
const styles = StyleSheet.create({
  ...
  inputIconRow: {
    flexDirection: 'row', alignItems: 'center',
    borderWidth: 1, paddingHorizontal: 14, paddingVertical: 13,
  },
  ...
  rateToggle: {
    flexDirection: 'row', borderWidth: 1, padding: 3, gap: 2, alignSelf: 'stretch',
  },
  rateToggleBtn: {
